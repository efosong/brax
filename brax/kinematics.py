...
def forward(sys: System, q: jax.Array, qd: jax.Array) -> Tuple[Transform, Motion]:
...
def link_to_joint_frame(motion: Motion) -> Tuple[Motion, float]:
    ...
    return (
        Motion(ang=ang_frame, vel=vel_frame),
        parity,
...
def inverse(sys: System, j: Transform, jd: Motion) -> Tuple[jax.Array, jax.Array]:
    ...
    def q_fn(typ, j, jd, parent_idx, motion):
        motion = jax.tree.map(lambda y: y.reshape((-1, base.QD_WIDTHS[typ], 3)), motion)
        q_fn_map = {
            "f": free,
